...
int main () {
    ...
    my_map<long long, long long> map(2*n+7);

    for (int i = 0; i < n; i++) {
        long long bucket_id = v[i] / w;

        if (map.has_key(bucket_id)) {
            cout << "SI\n";
            return 0;
        }

        if (map.has_key(bucket_id - 1) && abs(v[i] - map.get(bucket_id - 1)) <= t) {
            cout << "SI\n";
            return 0;
        }
        ...
        if (i >= k) {
            long long old_bucket = v[i - k] / w;
            map.erase(old_bucket);
        }
    }
